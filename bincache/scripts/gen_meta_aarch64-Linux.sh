...
generate_meta()
{
 ...
 #Check if contains needed fields
  if jq -e 'map(select(.ghcr_pkg != null and .ghcr_pkg != "")) | length > 0' "${TMPDIR}/${TMPJSON}" > /dev/null; then
   #Get Tag
     PKG_TAG="$(oras repo tags "ghcr.io/pkgforge/${PKG}" | grep -vE '^latest\.[0-9]{6}T[0-9]{6}\.' | grep -i "aarch64-Linux" | tail -n 1 | tr -d '[:space:]')"
   #Check Tag 
...
#Check missing (pkg_id)
  jq '.[] | select(.pkg_id == null or .pkg_id == "" or .pkg_id == "null") | {pkg, build_script}' "${TMPDIR}/bincache_aarch64-Linux.json.tmp" > "${SYSTMP}/MISSING_PKG_ID.json"
#Check dupes (pkg_webpage)
  jq 'group_by(.pkg_webpage) | map(select(length > 1)) | flatten | map({pkg_webpage: .pkg_webpage,build_script: .build_script})' "${TMPDIR}/bincache_aarch64-Linux.json.tmp" > "${SYSTMP}/DUPES_PKG_WEBPAGE.json"
##Sort Rank
jq '
 def compute_ranks:
  ...
  map(.value) | sort_by(.pkg)' | jq '.[] | .download_count |= tostring' | jq 'walk(if type == "boolean" then tostring else . end)' | jq -s 'if type == "array" then . else [.] end' | jq 'unique_by(.ghcr_pkg) | sort_by(.pkg)' > "${TMPDIR}/bincache_aarch64-Linux.json"
...
if command -v rclone &> /dev/null &&\
 ...
 #To SDB
  soarql --repo "bincache" --input "${GITHUB_WORKSPACE}/main/bincache/data/aarch64-Linux.json" --output "${GITHUB_WORKSPACE}/main/bincache/data/aarch64-Linux.sdb"
   if [[ $(stat -c%s "${GITHUB_WORKSPACE}/main/bincache/data/aarch64-Linux.sdb") -le 1024 ]] || file -i "${GITHUB_WORKSPACE}/main/bincache/data/aarch64-Linux.sdb" | grep -qiv 'sqlite'; then
 ...
 #To xz
  ...
  7z a -t7z -mx=9 -mmt="$(($(nproc)+1))" -bsp1 -bt "aarch64-Linux.sdb.xz" "aarch64-Linux.sdb" 2>/dev/null ; generate_checksum "aarch64-Linux.sdb.xz"
 #To Zstd
 ...
 #Upload (Json)
  ...
  rclone copyto "${GITHUB_WORKSPACE}/main/bincache/data/aarch64-Linux.json.zstd.bsum" "r2:/meta/bincache/aarch64-linux.json.zstd.bsum" --checksum --check-first --user-agent="${USER_AGENT}" &
  #Upload (SDB)
  rclone copyto "${GITHUB_WORKSPACE}/main/bincache/data/aarch64-Linux.sdb" "r2:/meta/bincache/aarch64-Linux.sdb" --checksum --check-first --user-agent="${USER_AGENT}" &
  rclone copyto "${GITHUB_WORKSPACE}/main/bincache/data/aarch64-Linux.sdb" "r2:/meta/bincache/aarch64-linux.sdb" --checksum --check-first --user-agent="${USER_AGENT}" &
  rclone copyto "${GITHUB_WORKSPACE}/main/bincache/data/aarch64-Linux.sdb.bsum" "r2:/meta/bincache/aarch64-Linux.sdb.bsum" --checksum --check-first --user-agent="${USER_AGENT}" &
  rclone copyto "${GITHUB_WORKSPACE}/main/bincache/data/aarch64-Linux.sdb.bsum" "r2:/meta/bincache/aarch64-linux.sdb.bsum" --checksum --check-first --user-agent="${USER_AGENT}" &
  rclone copyto "${GITHUB_WORKSPACE}/main/bincache/data/aarch64-Linux.sdb.cba" "r2:/meta/bincache/aarch64-Linux.sdb.cba" --checksum --check-first --user-agent="${USER_AGENT}" &
  rclone copyto "${GITHUB_WORKSPACE}/main/bincache/data/aarch64-Linux.sdb.cba" "r2:/meta/bincache/aarch64-linux.sdb.cba" --checksum --check-first --user-agent="${USER_AGENT}" &
  rclone copyto "${GITHUB_WORKSPACE}/main/bincache/data/aarch64-Linux.sdb.xz" "r2:/meta/bincache/aarch64-Linux.sdb.xz" --checksum --check-first --user-agent="${USER_AGENT}" &
  rclone copyto "${GITHUB_WORKSPACE}/main/bincache/data/aarch64-Linux.sdb.xz" "r2:/meta/bincache/aarch64-linux.sdb.xz" --checksum --check-first --user-agent="${USER_AGENT}" &
  rclone copyto "${GITHUB_WORKSPACE}/main/bincache/data/aarch64-Linux.sdb.xz.bsum" "r2:/meta/bincache/aarch64-Linux.sdb.xz.bsum" --checksum --check-first --user-agent="${USER_AGENT}" &
  rclone copyto "${GITHUB_WORKSPACE}/main/bincache/data/aarch64-Linux.sdb.xz.bsum" "r2:/meta/bincache/aarch64-linux.sdb.xz.bsum" --checksum --check-first --user-agent="${USER_AGENT}" &
  rclone copyto "${GITHUB_WORKSPACE}/main/bincache/data/aarch64-Linux.sdb.zstd" "r2:/meta/bincache/aarch64-Linux.sdb.zstd" --checksum --check-first --user-agent="${USER_AGENT}" &
  rclone copyto "${GITHUB_WORKSPACE}/main/bincache/data/aarch64-Linux.sdb.zstd" "r2:/meta/bincache/aarch64-linux.sdb.zstd" --checksum --check-first --user-agent="${USER_AGENT}" &
  rclone copyto "${GITHUB_WORKSPACE}/main/bincache/data/aarch64-Linux.sdb.zstd.bsum" "r2:/meta/bincache/aarch64-Linux.sdb.zstd.bsum" --checksum --check-first --user-agent="${USER_AGENT}" &
  rclone copyto "${GITHUB_WORKSPACE}/main/bincache/data/aarch64-Linux.sdb.zstd.bsum" "r2:/meta/bincache/aarch64-linux.sdb.zstd.bsum" --checksum --check-first --user-agent="${USER_AGENT}" &
  wait ; echo
fi
#-------------------------------------------------------#
