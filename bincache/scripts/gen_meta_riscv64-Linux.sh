...
## Meant to Generate Primary Metadata for bincache:: "${SYSTMP}/bincache_riscv64-Linux.json"
## Self: https://raw.githubusercontent.com/pkgforge/metadata/refs/heads/main/bincache/scripts/gen_meta_riscv64-Linux.sh
# PARALLEL_LIMIT="20" bash <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/metadata/refs/heads/main/bincache/scripts/gen_meta_riscv64-Linux.sh")
#-------------------------------------------------------#
...
rm -rvf "${SYSTMP}/bincache_riscv64-Linux.json" 2>/dev/null
...
echo -e "\n[+] Fetching Package List <== https://github.com/orgs/pkgforge/packages\n"
 for i in {1..5}; do
   ...
   if [[ "${PKG_GH_TMP}" -lt 50 ]]; then
     ...
     jq -r '.[] | select(.visibility=="public") | .name' "${TMPDIR}/ghcr_pkgs.tmp.json" | grep -Eiv '.*-srcbuild(-.*|$)' | grep -i 'bincache' | sort -u -o "${TMPDIR}/ghcr_pkgs.tmp"
     unset PKG_GH_TMP
     break
   fi
 done
...
generate_meta()
{
 ...
 #Check if contains needed fields
  if jq -e 'map(select(.ghcr_pkg != null and .ghcr_pkg != "")) | length > 0' "${TMPDIR}/${TMPJSON}" > /dev/null; then
   #Get Tag
     PKG_TAG="$(oras repo tags "ghcr.io/pkgforge/${PKG}" | grep -viE '^\s*(latest|srcbuild)[.-][0-9]{6}T[0-9]{6}[.-]' | grep -i "riscv64-Linux" | tail -n 1 | tr -d '[:space:]')"
   #Check Tag 
          ...
          if jq -r '.ghcr_pkg' "${TMPDIR}/${METADATA_JSON}.tmp01" | grep -q "${PKG}"; then
           ...
           #Add/Update version comparison
            ...
            jq --argjson target "$(jq '.build_script' "${TMPDIR}/${METADATA_JSON}.tmp01")" \
               --arg HOST_TRIPLET "${HOST_TRIPLET}" --arg REPO "bincache" \
...
#-------------------------------------------------------#
##Merge
find "${TMPDIR}" -type f -name '*-metadata.json' -exec bash -c 'jq empty "{}" 2>/dev/null && cat "{}"' \; | jq -s 'sort_by(.pkg)' > "${TMPDIR}/bincache_riscv64-Linux.json.tmp"
#Check missing (pkg_id)
  jq '.[] | select(.pkg_id == null or .pkg_id == "" or .pkg_id == "null") | {pkg, build_script}' "${TMPDIR}/bincache_riscv64-Linux.json.tmp" > "${SYSTMP}/MISSING_PKG_ID.json"
#Check dupes (pkg_webpage)
  jq 'group_by(.pkg_webpage) | map(select(length > 1)) | flatten | map({pkg_webpage: .pkg_webpage,build_script: .build_script})' "${TMPDIR}/bincache_riscv64-Linux.json.tmp" > "${SYSTMP}/DUPES_PKG_WEBPAGE.json"
##Sort Rank
jq '
 def compute_ranks:
  ...
  )' "${TMPDIR}/bincache_riscv64-Linux.json.tmp" | jq '
  ...
  ))' | jq 'unique_by(.ghcr_pkg) | sort_by(.pkg)' > "${TMPDIR}/bincache_riscv64-Linux.json"
#sanity check rank : jq 'sort_by(.rank | tonumber) | map({pkg_name, rank, download_count})'
#sanity check urls
sed -E 's~\bhttps?:/{1,2}\b~https://~g' -i "${TMPDIR}/bincache_riscv64-Linux.json"
##Check
unset PKG_COUNT; PKG_COUNT="$(jq -r '.[] | .ghcr_pkg' "${TMPDIR}/bincache_riscv64-Linux.json" | sort -u | wc -l | tr -d '[:space:]')"
if [[ "${PKG_COUNT}" -le 200 ]]; then
 echo -e "\n[X] FATAL: Final Package Count is < 200, Parsing Failed?\n"
 exit 1
else
 echo -e "\n[+] Packages (Bincache): ${PKG_COUNT}"
 mv -fv "${TMPDIR}/bincache_riscv64-Linux.json" "${SYSTMP}/bincache_riscv64-Linux.json"
fi
#-------------------------------------------------------#

#-------------------------------------------------------#
##Copy to "${GITHUB_WORKSPACE}/main/bincache/data"
if [ -s "${SYSTMP}/bincache_riscv64-Linux.json" ] &&\
 [ -d "${GITHUB_WORKSPACE}" ] &&\
 ...
 #Copy
  ...
  cp -fv "${SYSTMP}/bincache_riscv64-Linux.json" "${GITHUB_WORKSPACE}/main/bincache/data/riscv64-Linux.json"
  #Checksum
  generate_checksum() 
  {
      b3sum "$1" | grep -oE '^[a-f0-9]{64}' | tr -d '[:space:]' > "$1.bsum"
  }
  generate_checksum "riscv64-Linux.json"
 #To SDB
  soarql --repo "bincache" --input "${GITHUB_WORKSPACE}/main/bincache/data/riscv64-Linux.json" --output "${GITHUB_WORKSPACE}/main/bincache/data/riscv64-Linux.sdb"
  generate_checksum "riscv64-Linux.sdb"
   if [[ $(stat -c%s "${GITHUB_WORKSPACE}/main/bincache/data/riscv64-Linux.sdb") -le 1024 ]] || file -i "${GITHUB_WORKSPACE}/main/bincache/data/riscv64-Linux.sdb" | grep -qiv 'sqlite'; then
     echo -e "\n[✗] FATAL: Failed to generate Soar DB...\n"
   exit 1
   fi
 #To Bita
  bita compress --input "riscv64-Linux.json" --compression "zstd" --compression-level "21" --force-create "riscv64-Linux.json.cba"
  bita compress --input "riscv64-Linux.sdb" --compression "zstd" --compression-level "21" --force-create "riscv64-Linux.sdb.cba"
 #To Sqlite
  if command -v "qsv" >/dev/null 2>&1; then
    jq -c '.[]' "riscv64-Linux.json" > "${TMPDIR}/riscv64-Linux.jsonl"
    qsv jsonl "${TMPDIR}/riscv64-Linux.jsonl" > "${TMPDIR}/riscv64-Linux.csv"
    qsv to sqlite "${TMPDIR}/riscv64-Linux.db" "${TMPDIR}/riscv64-Linux.csv"
    if [[ -s "${TMPDIR}/riscv64-Linux.db" && $(stat -c%s "${TMPDIR}/riscv64-Linux.db") -gt 1024 ]]; then
     cp -fv "${TMPDIR}/riscv64-Linux.db" "${GITHUB_WORKSPACE}/main/bincache/data/riscv64-Linux.db" ; generate_checksum "riscv64-Linux.db"
     bita compress --input "${GITHUB_WORKSPACE}/main/bincache/data/riscv64-Linux.db" --compression "zstd" --compression-level "21" --force-create "${GITHUB_WORKSPACE}/main/bincache/data/riscv64-Linux.db.cba"
     7z a -t7z -mx="9" -mmt="$(($(nproc)+1))" -bsp1 -bt "${GITHUB_WORKSPACE}/main/bincache/data/riscv64-Linux.db.xz" "${GITHUB_WORKSPACE}/main/bincache/data/riscv64-Linux.db" 2>/dev/null ; generate_checksum "riscv64-Linux.db.xz"
     zstd --ultra -22 --force "${GITHUB_WORKSPACE}/main/bincache/data/riscv64-Linux.db" -o "${GITHUB_WORKSPACE}/main/bincache/data/riscv64-Linux.db.zstd" ; generate_checksum "riscv64-Linux.db.zstd"
     ##Upload
     # rclone copyto "${GITHUB_WORKSPACE}/main/bincache/data/riscv64-Linux.db" "r2:/meta/bincache/riscv64-Linux.db" --checksum --check-first --user-agent="${USER_AGENT}" &
     # rclone copyto "${GITHUB_WORKSPACE}/main/bincache/data/riscv64-Linux.db.bsum" "r2:/meta/bincache/riscv64-Linux.db.bsum" --checksum --check-first --user-agent="${USER_AGENT}" &
     # rclone copyto "${GITHUB_WORKSPACE}/main/bincache/data/riscv64-Linux.db.cba" "r2:/meta/bincache/riscv64-Linux.db.cba" --checksum --check-first --user-agent="${USER_AGENT}" &
     # rclone copyto "${GITHUB_WORKSPACE}/main/bincache/data/riscv64-Linux.db.xz" "r2:/meta/bincache/riscv64-Linux.db.xz" --checksum --check-first --user-agent="${USER_AGENT}" &
     # rclone copyto "${GITHUB_WORKSPACE}/main/bincache/data/riscv64-Linux.db.xz.bsum" "r2:/meta/bincache/riscv64-Linux.db.xz.bsum" --checksum --check-first --user-agent="${USER_AGENT}" &
     # rclone copyto "${GITHUB_WORKSPACE}/main/bincache/data/riscv64-Linux.db.zstd" "r2:/meta/bincache/riscv64-Linux.db.zstd" --checksum --check-first --user-agent="${USER_AGENT}" &
     # rclone copyto "${GITHUB_WORKSPACE}/main/bincache/data/riscv64-Linux.db.zstd.bsum" "r2:/meta/bincache/riscv64-Linux.db.zstd.bsum" --checksum --check-first --user-agent="${USER_AGENT}" &
    fi
  fi
 #To xz
  xz -9 -T"$(($(nproc) + 1))" --compress --extreme --keep --force --verbose "riscv64-Linux.json" ; generate_checksum "riscv64-Linux.json.xz"
  xz -9 -T"$(($(nproc) + 1))" --compress --extreme --keep --force --verbose "riscv64-Linux.sdb" ; generate_checksum "riscv64-Linux.sdb.xz"
 #To Zstd
  zstd --ultra -22 --force "riscv64-Linux.json" -o "riscv64-Linux.json.zstd" ; generate_checksum "riscv64-Linux.json.zstd"
  zstd --ultra -22 --force "riscv64-Linux.sdb" -o "riscv64-Linux.sdb.zstd" ; generate_checksum "riscv64-Linux.sdb.zstd"
 ##Upload (Json)
  #rclone copyto "${GITHUB_WORKSPACE}/main/bincache/data/riscv64-Linux.json" "r2:/meta/bincache/riscv64-Linux.json" --checksum --check-first --user-agent="${USER_AGENT}" &
  #rclone copyto "${GITHUB_WORKSPACE}/main/bincache/data/riscv64-Linux.json.bsum" "r2:/meta/bincache/riscv64-Linux.json.bsum" --checksum --check-first --user-agent="${USER_AGENT}" &
  #rclone copyto "${GITHUB_WORKSPACE}/main/bincache/data/riscv64-Linux.json.cba" "r2:/meta/bincache/riscv64-Linux.json.cba" --checksum --check-first --user-agent="${USER_AGENT}" &
  #rclone copyto "${GITHUB_WORKSPACE}/main/bincache/data/riscv64-Linux.json.xz" "r2:/meta/bincache/riscv64-Linux.json.xz" --checksum --check-first --user-agent="${USER_AGENT}" &
  #rclone copyto "${GITHUB_WORKSPACE}/main/bincache/data/riscv64-Linux.json.xz.bsum" "r2:/meta/bincache/riscv64-Linux.json.xz.bsum" --checksum --check-first --user-agent="${USER_AGENT}" &
  #rclone copyto "${GITHUB_WORKSPACE}/main/bincache/data/riscv64-Linux.json.zstd" "r2:/meta/bincache/riscv64-Linux.json.zstd" --checksum --check-first --user-agent="${USER_AGENT}" &
  #rclone copyto "${GITHUB_WORKSPACE}/main/bincache/data/riscv64-Linux.json.zstd.bsum" "r2:/meta/bincache/riscv64-Linux.json.zstd.bsum" --checksum --check-first --user-agent="${USER_AGENT}" &
  ##Upload (SDB)
  #rclone copyto "${GITHUB_WORKSPACE}/main/bincache/data/riscv64-Linux.sdb" "r2:/meta/bincache/riscv64-Linux.sdb" --checksum --check-first --user-agent="${USER_AGENT}" &
  #rclone copyto "${GITHUB_WORKSPACE}/main/bincache/data/riscv64-Linux.sdb.bsum" "r2:/meta/bincache/riscv64-Linux.sdb.bsum" --checksum --check-first --user-agent="${USER_AGENT}" &
  #rclone copyto "${GITHUB_WORKSPACE}/main/bincache/data/riscv64-Linux.sdb.cba" "r2:/meta/bincache/riscv64-Linux.sdb.cba" --checksum --check-first --user-agent="${USER_AGENT}" &
  #rclone copyto "${GITHUB_WORKSPACE}/main/bincache/data/riscv64-Linux.sdb.xz" "r2:/meta/bincache/riscv64-Linux.sdb.xz" --checksum --check-first --user-agent="${USER_AGENT}" &
  #rclone copyto "${GITHUB_WORKSPACE}/main/bincache/data/riscv64-Linux.sdb.xz.bsum" "r2:/meta/bincache/riscv64-Linux.sdb.xz.bsum" --checksum --check-first --user-agent="${USER_AGENT}" &
  #rclone copyto "${GITHUB_WORKSPACE}/main/bincache/data/riscv64-Linux.sdb.zstd" "r2:/meta/bincache/riscv64-Linux.sdb.zstd" --checksum --check-first --user-agent="${USER_AGENT}" &
  #rclone copyto "${GITHUB_WORKSPACE}/main/bincache/data/riscv64-Linux.sdb.zstd.bsum" "r2:/meta/bincache/riscv64-Linux.sdb.zstd.bsum" --checksum --check-first --user-agent="${USER_AGENT}" &
  #wait ; echo
fi
#-------------------------------------------------------#
