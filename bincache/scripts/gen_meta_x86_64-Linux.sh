...
jq '
 def compute_ranks:
  ...
  map(.value) | sort_by(.pkg)' | jq '.[] | .download_count |= tostring' | jq -s 'if type == "array" then . else [.] end' > "${TMPDIR}/bincache_x86_64-Linux.json"
 ...
 #To Sqlite
  if command -v "qsv" >/dev/null 2>&1; then
    ...
    if [[ -s "${TMPDIR}/x86_64-Linux.db" && $(stat -c%s "${TMPDIR}/x86_64-Linux.db") -gt 1024 ]]; then
     ...
     bita compress --input "x86_64-Linux.db" --compression "zstd" --compression-level "21" --force-create "x86_64-Linux.db.cba"
     7z a -t7z -mx="9" -mmt="$(($(nproc)+1))" -bsp1 -bt "${GITHUB_WORKSPACE}/main/aarch64-Linux/x86_64-Linux.db.xz" "${GITHUB_WORKSPACE}/main/aarch64-Linux/x86_64-Linux.db" 2>/dev/null ; generate_checksum "x86_64-Linux.db.xz"
     zstd --ultra -22 --force "${GITHUB_WORKSPACE}/main/aarch64-Linux/x86_64-Linux.db" -o "${GITHUB_WORKSPACE}/main/aarch64-Linux/x86_64-Linux.db.zstd" ; generate_checksum "x86_64-Linux.db.zstd"
     #Upload
