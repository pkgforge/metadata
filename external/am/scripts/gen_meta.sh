...
if command -v rclone &> /dev/null &&\
 ...
 #Gen & Upload AM (HF-Mirror-Only) [aarch64-Linux]
  ...
  curl -qfsSL "https://github.com/pkgforge/bincache/releases/download/metadata/x86_64-Linux.json" | jq -r '.[] | "| \(.pkg_name)#\(.pkg_id) | \(.description) | \(((.src_url[0] // .homepage[0]) // "N/A")) | \(.download_url) | \((if .version then .version else (.bsum // "latest")[:12] end)) |"' >> "${SYSTMP}/aarch64-Linux.AM.txt"
 ...
 #Gen & Upload AM (HF-Mirror-Only) [x86_64-Linux]
  ...
  curl -qfsSL "https://github.com/pkgforge/pkgcache/releases/download/metadata/aarch64-Linux.json" | jq -r '.[] | "| \(.pkg_name)#\(.pkg_id) | \(.description) | \(((.src_url[0] // .homepage[0]) // "N/A")) | \(.download_url) | \((if .version then .version else (.bsum // "latest")[:12] end)) |"' > "${SYSTMP}/x86_64-Linux.AM.txt"
