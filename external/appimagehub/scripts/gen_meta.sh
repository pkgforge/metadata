...
if command -v rclone &> /dev/null &&\
 ...
 #Copy
  ...
  jq -s 'map(.[]) | group_by(.pkg_id) | map(add)' "${SYSTMP}/appimagehub.json" "${GITHUB_WORKSPACE}/main/external/appimagehub/data/${HOST_TRIPLET}.json" | jq 'unique_by(.pkg_id) | sort_by(.pkg)' | jq . > "${SYSTMP}/merged.json"
