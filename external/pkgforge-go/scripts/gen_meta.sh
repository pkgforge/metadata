    ...
    #Copy
     if [[ -s "${SYSTMP}/${HOST_TRIPLET}.json" ]] &&\
      ...
      #Fix ID
       ...
       generate_checksum "${SYSTMP}/${HOST_TRIPLET}.json"
       generate_checksum "${SYSTMP}/${HOST_TRIPLET}.json.in"
       #To SDB
       soarql --repo "pkgforge-go" --input "${SYSTMP}/${HOST_TRIPLET}.json.in" --output "${SYSTMP}/${HOST_TRIPLET}.sdb"
       generate_checksum "${SYSTMP}/${HOST_TRIPLET}.sdb"
      ...
      #To xz
       xz -9 -T"$(($(nproc) + 1))" --compress --extreme --keep --force --verbose "${SYSTMP}/${HOST_TRIPLET}.json.in"
       mv -fv "${SYSTMP}/${HOST_TRIPLET}.json.in.xz" "${SYSTMP}/${HOST_TRIPLET}.json.xz"
       generate_checksum "${HOST_TRIPLET}.json.xz"
