...
## Meant to Generate Primary Metadata for pkgcache:: "${SYSTMP}/pkgcache_riscv64-Linux.json"
## Self: https://raw.githubusercontent.com/pkgforge/metadata/refs/heads/main/pkgcache/scripts/gen_meta_riscv64-Linux.sh
# PARALLEL_LIMIT="20" bash <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/metadata/refs/heads/main/pkgcache/scripts/gen_meta_riscv64-Linux.sh")
#-------------------------------------------------------#
...
rm -rvf "${SYSTMP}/pkgcache_riscv64-Linux.json" 2>/dev/null
...
#-------------------------------------------------------#
##Merge
find "${TMPDIR}" -type f -name '*-metadata.json' -exec bash -c 'jq empty "{}" 2>/dev/null && cat "{}"' \; | jq -s 'sort_by(.pkg)' > "${TMPDIR}/pkgcache_riscv64-Linux.json.tmp"
#Check missing (pkg_id)
  jq '.[] | select(.pkg_id == null or .pkg_id == "" or .pkg_id == "null") | {pkg, build_script}' "${TMPDIR}/pkgcache_riscv64-Linux.json.tmp" > "${SYSTMP}/MISSING_PKG_ID.json"
#Check dupes (pkg_webpage)
  jq 'group_by(.pkg_webpage) | map(select(length > 1)) | flatten | map({pkg_webpage: .pkg_webpage,build_script: .build_script})' "${TMPDIR}/pkgcache_riscv64-Linux.json.tmp" > "${SYSTMP}/DUPES_PKG_WEBPAGE.json"
##Sort Rank
jq '
 def compute_ranks:
  ...
  )' "${TMPDIR}/pkgcache_riscv64-Linux.json.tmp" | jq '
  ...
  ))' | jq 'unique_by(.ghcr_pkg) | sort_by(.pkg)' > "${TMPDIR}/pkgcache_riscv64-Linux.json"
#sanity check: jq 'sort_by(.rank | tonumber) | map({pkg_name, rank, download_count})'
#sanity check urls
sed -E 's~\bhttps?:/{1,2}\b~https://~g' -i "${TMPDIR}/pkgcache_riscv64-Linux.json"
##Check
unset PKG_COUNT; PKG_COUNT="$(jq -r '.[] | .ghcr_pkg' "${TMPDIR}/pkgcache_riscv64-Linux.json" | sort -u | wc -l | tr -d '[:space:]')"
if [[ "${PKG_COUNT}" -le 2 ]]; then
 echo -e "\n[X] FATAL: Final Package Count is < 2, Parsing Failed?\n"
 exit 1
else
 echo -e "\n[+] Packages (Pkgcache): ${PKG_COUNT}"
 mv -fv "${TMPDIR}/pkgcache_riscv64-Linux.json" "${SYSTMP}/pkgcache_riscv64-Linux.json"
fi
#-------------------------------------------------------#

#-------------------------------------------------------#
##Copy to "${GITHUB_WORKSPACE}/main/pkgcache/data"
if [ -s "${SYSTMP}/pkgcache_riscv64-Linux.json" ] &&\
 [ -d "${GITHUB_WORKSPACE}" ] &&\
 ...
 #Copy
  ...
  cp -fv "${SYSTMP}/pkgcache_riscv64-Linux.json" "${GITHUB_WORKSPACE}/main/pkgcache/data/riscv64-Linux.json"
  #Checksum
  generate_checksum() 
  {
 ...
 #To SDB
  soarql --repo "pkgcache" --input "${GITHUB_WORKSPACE}/main/pkgcache/data/riscv64-Linux.json" --output "${GITHUB_WORKSPACE}/main/pkgcache/data/riscv64-Linux.sdb"
  generate_checksum "riscv64-Linux.sdb"
   if [[ $(stat -c%s "${GITHUB_WORKSPACE}/main/pkgcache/data/riscv64-Linux.sdb") -le 1024 ]] || file -i "${GITHUB_WORKSPACE}/main/pkgcache/data/riscv64-Linux.sdb" | grep -qiv 'sqlite'; then
 ...
 #To Sqlite
  if command -v "qsv" >/dev/null 2>&1; then
    ...
    if [[ -s "${TMPDIR}/riscv64-Linux.db" && $(stat -c%s "${TMPDIR}/riscv64-Linux.db") -gt 1024 ]]; then
     cp -fv "${TMPDIR}/riscv64-Linux.db" "${GITHUB_WORKSPACE}/main/pkgcache/data/riscv64-Linux.db" ; generate_checksum "riscv64-Linux.db"
     bita compress --input "${GITHUB_WORKSPACE}/main/pkgcache/data/riscv64-Linux.db" --compression "zstd" --compression-level "21" --force-create "${GITHUB_WORKSPACE}/main/pkgcache/data/riscv64-Linux.db.cba"
     7z a -t7z -mx="9" -mmt="$(($(nproc)+1))" -bsp1 -bt "${GITHUB_WORKSPACE}/main/pkgcache/data/riscv64-Linux.db.xz" "${GITHUB_WORKSPACE}/main/pkgcache/data/riscv64-Linux.db" 2>/dev/null ; generate_checksum "riscv64-Linux.db.xz"
     zstd --ultra -22 --force "${GITHUB_WORKSPACE}/main/pkgcache/data/riscv64-Linux.db" -o "${GITHUB_WORKSPACE}/main/pkgcache/data/riscv64-Linux.db.zstd" ; generate_checksum "riscv64-Linux.db.zstd"
     ##Upload
      #rclone copyto "${GITHUB_WORKSPACE}/main/pkgcache/data/riscv64-Linux.db" "r2:/meta/pkgcache/riscv64-Linux.db" --checksum --check-first --user-agent="${USER_AGENT}" &
      #rclone copyto "${GITHUB_WORKSPACE}/main/pkgcache/data/riscv64-Linux.db.bsum" "r2:/meta/pkgcache/riscv64-Linux.db.bsum" --checksum --check-first --user-agent="${USER_AGENT}" &
      #rclone copyto "${GITHUB_WORKSPACE}/main/pkgcache/data/riscv64-Linux.db.cba" "r2:/meta/pkgcache/riscv64-Linux.db.cba" --checksum --check-first --user-agent="${USER_AGENT}" &
      #rclone copyto "${GITHUB_WORKSPACE}/main/pkgcache/data/riscv64-Linux.db.xz" "r2:/meta/pkgcache/riscv64-Linux.db.xz" --checksum --check-first --user-agent="${USER_AGENT}" &
      #rclone copyto "${GITHUB_WORKSPACE}/main/pkgcache/data/riscv64-Linux.db.xz.bsum" "r2:/meta/pkgcache/riscv64-Linux.db.xz.bsum" --checksum --check-first --user-agent="${USER_AGENT}" &
      #rclone copyto "${GITHUB_WORKSPACE}/main/pkgcache/data/riscv64-Linux.db.zstd" "r2:/meta/pkgcache/riscv64-Linux.db.zstd" --checksum --check-first --user-agent="${USER_AGENT}" &
      #rclone copyto "${GITHUB_WORKSPACE}/main/pkgcache/data/riscv64-Linux.db.zstd.bsum" "r2:/meta/pkgcache/riscv64-Linux.db.zstd.bsum" --checksum --check-first --user-agent="${USER_AGENT}" &
    fi
  fi
 #To xz
 ...
 ##Upload (Json)
  #rclone copyto "${GITHUB_WORKSPACE}/main/pkgcache/data/riscv64-Linux.json" "r2:/meta/pkgcache/riscv64-Linux.json" --checksum --check-first --user-agent="${USER_AGENT}" &
  #rclone copyto "${GITHUB_WORKSPACE}/main/pkgcache/data/riscv64-Linux.json.bsum" "r2:/meta/pkgcache/riscv64-Linux.json.bsum" --checksum --check-first --user-agent="${USER_AGENT}" &
  #rclone copyto "${GITHUB_WORKSPACE}/main/pkgcache/data/riscv64-Linux.json.cba" "r2:/meta/pkgcache/riscv64-Linux.json.cba" --checksum --check-first --user-agent="${USER_AGENT}" &
  #rclone copyto "${GITHUB_WORKSPACE}/main/pkgcache/data/riscv64-Linux.json.xz" "r2:/meta/pkgcache/riscv64-Linux.json.xz" --checksum --check-first --user-agent="${USER_AGENT}" &
  #rclone copyto "${GITHUB_WORKSPACE}/main/pkgcache/data/riscv64-Linux.json.xz.bsum" "r2:/meta/pkgcache/riscv64-Linux.json.xz.bsum" --checksum --check-first --user-agent="${USER_AGENT}" &
  #rclone copyto "${GITHUB_WORKSPACE}/main/pkgcache/data/riscv64-Linux.json.zstd" "r2:/meta/pkgcache/riscv64-Linux.json.zstd" --checksum --check-first --user-agent="${USER_AGENT}" &
  #rclone copyto "${GITHUB_WORKSPACE}/main/pkgcache/data/riscv64-Linux.json.zstd.bsum" "r2:/meta/pkgcache/riscv64-Linux.json.zstd.bsum" --checksum --check-first --user-agent="${USER_AGENT}" &
  ##Upload (SDB)
  #rclone copyto "${GITHUB_WORKSPACE}/main/pkgcache/data/riscv64-Linux.sdb" "r2:/meta/pkgcache/riscv64-Linux.sdb" --checksum --check-first --user-agent="${USER_AGENT}" &
  #rclone copyto "${GITHUB_WORKSPACE}/main/pkgcache/data/riscv64-Linux.sdb.bsum" "r2:/meta/pkgcache/riscv64-Linux.sdb.bsum" --checksum --check-first --user-agent="${USER_AGENT}" &
  #rclone copyto "${GITHUB_WORKSPACE}/main/pkgcache/data/riscv64-Linux.sdb.cba" "r2:/meta/pkgcache/riscv64-Linux.sdb.cba" --checksum --check-first --user-agent="${USER_AGENT}" &
  #rclone copyto "${GITHUB_WORKSPACE}/main/pkgcache/data/riscv64-Linux.sdb.xz" "r2:/meta/pkgcache/riscv64-Linux.sdb.xz" --checksum --check-first --user-agent="${USER_AGENT}" &
  #rclone copyto "${GITHUB_WORKSPACE}/main/pkgcache/data/riscv64-Linux.sdb.xz.bsum" "r2:/meta/pkgcache/riscv64-Linux.sdb.xz.bsum" --checksum --check-first --user-agent="${USER_AGENT}" &
  #rclone copyto "${GITHUB_WORKSPACE}/main/pkgcache/data/riscv64-Linux.sdb.zstd" "r2:/meta/pkgcache/riscv64-Linux.sdb.zstd" --checksum --check-first --user-agent="${USER_AGENT}" &
  #rclone copyto "${GITHUB_WORKSPACE}/main/pkgcache/data/riscv64-Linux.sdb.zstd.bsum" "r2:/meta/pkgcache/riscv64-Linux.sdb.zstd.bsum" --checksum --check-first --user-agent="${USER_AGENT}" &
  wait ; echo
fi
#-------------------------------------------------------#
