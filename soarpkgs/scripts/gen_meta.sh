...
for SBUILD in "${VALID_BINS[@]}"; do
    ...
    BINCACHE="$(jq --arg VALID_BINSRC "${VALID_BINSRC}" -r '.[] | select(.build_script | test($VALID_BINSRC+"$")) | .ghcr_pkg' "${TMPDIR}/bincache.json" | head -n 1 | awk -F'/' '{print $1"/"$2"/"$4"/"$5"/"$6}' | tr -d '[:space:]')"
...
for SBUILD in "${VALID_PKGS[@]}"; do
    ...
    BINCACHE="$(jq --arg VALID_PKGSRC "${VALID_PKGSRC}" -r '.[] | select(.build_script | test($VALID_PKGSRC+"$")) | .ghcr_pkg' "${TMPDIR}/bincache.json" | head -n 1 | awk -F'/' '{print $1"/"$2"/"$4"/"$5"/"$6}' | tr -d '[:space:]')"
...
#-------------------------------------------------------#
##Copy to ${SYSTMP}
jq -s add "${TMPDIR}/BINARIES.json" "${TMPDIR}/PACKAGES.json" | jq 'sort_by(.pkg)' | jq 'walk(if type == "object" then with_entries(select(.value != null and .value != "" and .value != [] and .value != {})) else . end)' | jq 'if type == "array" then . else [.] end' > "${TMPDIR}/ALL.json"
if [[ "$(jq -r '.[] | .build_script' "${TMPDIR}/ALL.json" | grep -iv 'null' | wc -l)" -le 2000 ]]; then
   echo -e "\n[-] FATAL: Failed to Generate Soarpkgs MetaData\n"
   exit 1   
else
   cp -fv "${TMPDIR}/ALL.json" "${SYSTMP}/ALL.json"
fi
#-------------------------------------------------------#
...
if command -v rclone &> /dev/null &&\
 ...
 #Copy
  ...
  cp -fv "${SYSTMP}/ALL.json" "${GITHUB_WORKSPACE}/main/soarpkgs/data/ALL.json"
  #Checksum
  generate_checksum() 
  {
      b3sum "$1" | grep -oE '^[a-f0-9]{64}' | tr -d '[:space:]' > "$1.bsum"
  }
  generate_checksum "ALL.json"
 #To Bita
  bita compress --input "ALL.json" --compression "zstd" --compression-level "21" --force-create "ALL.json.cba"
 #To Sqlite
  if command -v "qsv" >/dev/null 2>&1; then
    jq -c '.[]' "ALL.json" > "${TMPDIR}/ALL.jsonl"
    qsv jsonl "${TMPDIR}/ALL.jsonl" > "${TMPDIR}/ALL.csv"
    qsv to sqlite "${TMPDIR}/ALL.db" "${TMPDIR}/ALL.csv"
    if [[ -s "${TMPDIR}/ALL.db" && $(stat -c%s "${TMPDIR}/ALL.db") -gt 1024 ]]; then
     cp -fv "${TMPDIR}/ALL.db" "${GITHUB_WORKSPACE}/main/soarpkgs/data/ALL.db" ; generate_checksum "ALL.db"
     bita compress --input "${GITHUB_WORKSPACE}/main/soarpkgs/data/ALL.db" --compression "zstd" --compression-level "21" --force-create "${GITHUB_WORKSPACE}/main/soarpkgs/data/ALL.db.cba"
     7z a -t7z -mx="9" -mmt="$(($(nproc)+1))" -bsp1 -bt "${GITHUB_WORKSPACE}/main/soarpkgs/data/ALL.db.xz" "${GITHUB_WORKSPACE}/main/soarpkgs/data/ALL.db" 2>/dev/null ; generate_checksum "ALL.db.xz"
     zstd --ultra -22 --force "${GITHUB_WORKSPACE}/main/soarpkgs/data/ALL.db" -o "${GITHUB_WORKSPACE}/main/soarpkgs/data/ALL.db.zstd" ; generate_checksum "ALL.db.zstd"
     #Upload
      rclone copyto "${GITHUB_WORKSPACE}/main/soarpkgs/data/ALL.db" "r2:/meta/soarpkgs/ALL.db" --checksum --check-first --user-agent="${USER_AGENT}" &
      rclone copyto "${GITHUB_WORKSPACE}/main/soarpkgs/data/ALL.db" "r2:/meta/soarpkgs/ALL.db" --checksum --check-first --user-agent="${USER_AGENT}" &
      rclone copyto "${GITHUB_WORKSPACE}/main/soarpkgs/data/ALL.db.bsum" "r2:/meta/soarpkgs/ALL.db.bsum" --checksum --check-first --user-agent="${USER_AGENT}" &
      rclone copyto "${GITHUB_WORKSPACE}/main/soarpkgs/data/ALL.db.bsum" "r2:/meta/soarpkgs/ALL.db.bsum" --checksum --check-first --user-agent="${USER_AGENT}" &
      rclone copyto "${GITHUB_WORKSPACE}/main/soarpkgs/data/ALL.db.cba" "r2:/meta/soarpkgs/ALL.db.cba" --checksum --check-first --user-agent="${USER_AGENT}" &
      rclone copyto "${GITHUB_WORKSPACE}/main/soarpkgs/data/ALL.db.cba" "r2:/meta/soarpkgs/ALL.db.cba" --checksum --check-first --user-agent="${USER_AGENT}" &
      rclone copyto "${GITHUB_WORKSPACE}/main/soarpkgs/data/ALL.db.xz" "r2:/meta/soarpkgs/ALL.db.xz" --checksum --check-first --user-agent="${USER_AGENT}" &
      rclone copyto "${GITHUB_WORKSPACE}/main/soarpkgs/data/ALL.db.xz" "r2:/meta/soarpkgs/ALL.db.xz" --checksum --check-first --user-agent="${USER_AGENT}" &
      rclone copyto "${GITHUB_WORKSPACE}/main/soarpkgs/data/ALL.db.xz.bsum" "r2:/meta/soarpkgs/ALL.db.xz.bsum" --checksum --check-first --user-agent="${USER_AGENT}" &
      rclone copyto "${GITHUB_WORKSPACE}/main/soarpkgs/data/ALL.db.xz.bsum" "r2:/meta/soarpkgs/ALL.db.xz.bsum" --checksum --check-first --user-agent="${USER_AGENT}" &
      rclone copyto "${GITHUB_WORKSPACE}/main/soarpkgs/data/ALL.db.zstd" "r2:/meta/soarpkgs/ALL.db.zstd" --checksum --check-first --user-agent="${USER_AGENT}" &
      rclone copyto "${GITHUB_WORKSPACE}/main/soarpkgs/data/ALL.db.zstd" "r2:/meta/soarpkgs/ALL.db.zstd" --checksum --check-first --user-agent="${USER_AGENT}" &
      rclone copyto "${GITHUB_WORKSPACE}/main/soarpkgs/data/ALL.db.zstd.bsum" "r2:/meta/soarpkgs/ALL.db.zstd.bsum" --checksum --check-first --user-agent="${USER_AGENT}" &
      rclone copyto "${GITHUB_WORKSPACE}/main/soarpkgs/data/ALL.db.zstd.bsum" "r2:/meta/soarpkgs/ALL.db.zstd.bsum" --checksum --check-first --user-agent="${USER_AGENT}" &
    fi
  fi
 #To xz
  7z a -t7z -mx=9 -mmt="$(($(nproc)+1))" -bsp1 -bt "ALL.json.xz" "ALL.json" 2>/dev/null ; generate_checksum "ALL.json.xz"
 #To Zstd
  zstd --ultra -22 --force "ALL.json" -o "ALL.json.zstd" ; generate_checksum "ALL.json.zstd"
 #Upload
  rclone copyto "${GITHUB_WORKSPACE}/main/soarpkgs/data/ALL.json" "r2:/meta/soarpkgs/ALL.json" --checksum --check-first --user-agent="${USER_AGENT}" &
  rclone copyto "${GITHUB_WORKSPACE}/main/soarpkgs/data/ALL.json" "r2:/meta/soarpkgs/ALL.json" --checksum --check-first --user-agent="${USER_AGENT}" &
  rclone copyto "${GITHUB_WORKSPACE}/main/soarpkgs/data/ALL.json.bsum" "r2:/meta/soarpkgs/ALL.json.bsum" --checksum --check-first --user-agent="${USER_AGENT}" &
  rclone copyto "${GITHUB_WORKSPACE}/main/soarpkgs/data/ALL.json.bsum" "r2:/meta/soarpkgs/ALL.json.bsum" --checksum --check-first --user-agent="${USER_AGENT}" &
  rclone copyto "${GITHUB_WORKSPACE}/main/soarpkgs/data/ALL.json.cba" "r2:/meta/soarpkgs/ALL.json.cba" --checksum --check-first --user-agent="${USER_AGENT}" &
  rclone copyto "${GITHUB_WORKSPACE}/main/soarpkgs/data/ALL.json.cba" "r2:/meta/soarpkgs/ALL.json.cba" --checksum --check-first --user-agent="${USER_AGENT}" &
  rclone copyto "${GITHUB_WORKSPACE}/main/soarpkgs/data/ALL.json.xz" "r2:/meta/soarpkgs/ALL.json.xz" --checksum --check-first --user-agent="${USER_AGENT}" &
  rclone copyto "${GITHUB_WORKSPACE}/main/soarpkgs/data/ALL.json.xz" "r2:/meta/soarpkgs/ALL.json.xz" --checksum --check-first --user-agent="${USER_AGENT}" &
  rclone copyto "${GITHUB_WORKSPACE}/main/soarpkgs/data/ALL.json.xz.bsum" "r2:/meta/soarpkgs/ALL.json.xz.bsum" --checksum --check-first --user-agent="${USER_AGENT}" &
  rclone copyto "${GITHUB_WORKSPACE}/main/soarpkgs/data/ALL.json.xz.bsum" "r2:/meta/soarpkgs/ALL.json.xz.bsum" --checksum --check-first --user-agent="${USER_AGENT}" &
  rclone copyto "${GITHUB_WORKSPACE}/main/soarpkgs/data/ALL.json.zstd" "r2:/meta/soarpkgs/ALL.json.zstd" --checksum --check-first --user-agent="${USER_AGENT}" &
  rclone copyto "${GITHUB_WORKSPACE}/main/soarpkgs/data/ALL.json.zstd" "r2:/meta/soarpkgs/ALL.json.zstd" --checksum --check-first --user-agent="${USER_AGENT}" &
  rclone copyto "${GITHUB_WORKSPACE}/main/soarpkgs/data/ALL.json.zstd.bsum" "r2:/meta/soarpkgs/ALL.json.zstd.bsum" --checksum --check-first --user-agent="${USER_AGENT}" &
  rclone copyto "${GITHUB_WORKSPACE}/main/soarpkgs/data/ALL.json.zstd.bsum" "r2:/meta/soarpkgs/ALL.json.zstd.bsum" --checksum --check-first --user-agent="${USER_AGENT}" &
  wait ; echo
fi
#-------------------------------------------------------#