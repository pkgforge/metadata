...
# source <(curl -qfsSL "https://raw.githubusercontent.com/pkgforge/metadata/refs/heads/main/soarpkgs/scripts/repology_fetcher.sh")
#set -x
#-------------------------------------------------------#
...
repology_fetcher()
{
  ...
  ##Main
  if [[ "$(command -v awk)" && "$(command -v grep)" && "$(command -v jq)" && "$(command -v yq)" ]]; then 
    #ENV
     local INPUT="${1:-$(cat)}"
    ...
    #Fetch
     ...
     if [[ -s "${TMP_JSON}" ]] && [[ $(stat -c%s "${TMP_JSON}") -gt 100 ]]; then
       ...
       echo -e "[-] Exact Query ${REPOLOGY_PKG} Failed (Trying Fuzzy...)"
       {
        curl -A "${USER_AGENT}" -qfsSL "https://repology.org/api/v1/projects/?search=${REPOLOGY_PKG}" || curl -qfsSL "https://api.rl.pkgforge.dev/api/v1/projects/?search=${REPOLOGY_PKG}"
       } 2>/dev/null | jq -r 'keys[]' ; echo -e "\n"
     fi
  else
     echo "[-] Install: awk grep jq yq"
  fi
  ##Disable Debug 
   if [ "${DEBUG}" = "1" ] || [ "${DEBUG}" = "ON" ]; then
      set +x
